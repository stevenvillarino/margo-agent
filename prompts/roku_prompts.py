...
class RokuDesignPrompts:
    ...
    Compatibility wrapper for the new function-based approach.
    """
    ...
    def get_roku_evaluation_prompt(
        ...
        """Get a comprehensive Roku TV design evaluation prompt."""
        return get_roku_evaluation_prompt(design_context, focus_areas, include_grading)
    
    def get_figma_analysis_prompt(self) -> str:
        """Get Figma-specific analysis prompt."""
        return """
When analyzing this Figma design for Roku TV interface compliance:

1. Extract all visible text content and UI elements
2. Identify navigation patterns and content hierarchy
3. Assess visual design choices (colors, typography, spacing)
4. Note any accessibility considerations
5. Look for Roku-specific patterns or violations

Focus on elements that directly impact the user experience on a TV interface viewed from 10 feet away.
"""
    
    def get_confluence_extraction_prompt(self) -> str:
        """Get Confluence-specific extraction prompt."""
        return """
When analyzing Confluence pages for design documentation:

1. Extract design specifications and requirements
2. Identify user flows and interaction patterns
3. Note accessibility requirements
4. Look for Roku-specific guidelines or constraints
5. Extract any visual mockups or design references

Summarize the key design decisions and rationale that would impact a Roku TV interface.
"""
