...
This Figma integration is working! The design data has been extracted and is ready for comprehensive analysis by all our specialist agents.
    ...
    def _general_design_chat(self, user_input: str):
        ...
        # First, check if this is a brand/knowledge question
        if self._is_brand_question(user_input):
            self._handle_brand_question_with_agents(user_input)
            return
...
- **Brand Guidelines** - Roku colors, typography, voice & tone
- **User Experience** - Validate user flows and interactions

Just share your design or ask me anything! 🎨
            """
        else:
            # Check if this might be a knowledge gap that our agents should learn
            if self._might_be_knowledge_question(user_input):
                response = self._handle_knowledge_with_learning_system(user_input)
            else:
...
**For now:** Could you share a design file so I can provide specific feedback, or ask about general design principles?
        """
    
    def _is_brand_question(self, user_input: str) -> bool:
        """Check if user is asking about brand guidelines."""
        brand_keywords = [
            "brand color", "brand colours", "primary color", "purple", "roku purple",
            "brand font", "typography", "roku font", "brand guidelines", 
            "voice", "tone", "logo", "wordmark"
        ]
        return any(keyword in user_input.lower() for keyword in brand_keywords)
    
    def _might_be_knowledge_question(self, user_input: str) -> bool:
        """Check if this might be a question we should have knowledge about."""
        knowledge_indicators = [
            "what is", "what's", "tell me about", "explain", 
            "how do we", "what do we", "our team", "roku's",
            "company", "brand", "guidelines", "standards"
        ]
