...
CONFLUENCE_API_KEY=your_api_key_here""")
    ...
    with tab2:
        if not settings.is_figma_configured():
            st.warning("⚠️ Figma integration is not configured. Please add your FIGMA_ACCESS_TOKEN to the .env file.")
            st.code("FIGMA_ACCESS_TOKEN=your_token_here")
        else:
            ...
            with col1:
                ...
                figma_input_type = st.radio(
                    "Input Type",
                    ["Figma URL", "File Key"],
                    key="figma_input_type"
                )
                
                if figma_input_type == "Figma URL":
                    figma_url = st.text_input(
                        "Figma File URL",
                        placeholder="https://www.figma.com/file/your-file-key/...",
                        help="Paste the full Figma file URL"
                    )
                    file_key = ""
                    if figma_url:
                        try:
                    ...
                    file_key = st.text_input(
                        "Figma File Key",
                        placeholder="Enter the file key directly",
                        help="The file key from the Figma URL"
                    )
                
                node_ids_input = st.text_area(
                    "Node IDs (Optional)",
                    placeholder="Enter specific node IDs, one per line",
                    help="Leave empty to analyze the entire file"
                )
                
                node_ids = [nid.strip() for nid in node_ids_input.split('\n') if nid.strip()] if node_ids_input else None
            
            with col2:
                st.subheader("Figma Analysis")
                
                if file_key:
                    if st.button("🔍 Analyze Figma File", type="primary", key="figma_analyze"):
                        with st.spinner("Loading and analyzing Figma file..."):
                            try:
                                if evaluation_mode == "Roku TV Design Review":
                                    # Use Roku-specific evaluation
                                    review_result = agent.review_roku_design(
    ...
    with tab3:
        if not settings.is_confluence_configured():
            st.warning("⚠️ Confluence integration is not configured. Please add your Confluence credentials to the .env file.")
            st.code("""CONFLUENCE_URL=https://your-domain.atlassian.net
CONFLUENCE_USERNAME=your_email@domain.com
CONFLUENCE_API_KEY=your_api_key_here""")
        else:
